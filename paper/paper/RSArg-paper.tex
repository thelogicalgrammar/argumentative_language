...
Instead of calculating the maximin for a specific value of $\gamma$, we could also calculate it assuming that the agent is arguing _for_ "The value of $\gamma$ is greater than this threshold" and _against_ "The value of $\gamma$ is lower than this threshold". Formally, we can write it as follows:
...
- If we are in the low condition, the conservatively imagined listener will guess '12|12|12|12'
...
\FC{
  ...
  For several of the models, I calculated the posterior predictive p-values for all the models (see Bayesian p-value section above). In all cases they were quite close to 0.5, indicating good compatibility of the data with the fitted posterior. However, note that posterior predictive p-values are generally not uniformly distributed in [0,1] and hierarchical models pose a challenge (see e.g. [this paper](https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.165.118&rep=rep1&type=pdf)). 

It is also instructive to have a look at the _pointwise_ posterior predictive p-values. 
